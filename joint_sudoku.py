...
from sequence_sudoku import POLICIES, SOURCE, probe_metrics
...
def main():
    ...
    training = [{"operation": "place", "input": r["input"], "label": r["label"], "distinct_peers": r["distinct_peers"]}
                for r in place_contexts if r["policy"] == "capped-24" and r["distinct_peers"] > 0]
    ...
    protocol = {
        ...
        "undo_KC_ids": brain.ids[undo_groups].tolist(), "primary_policy": "capped-24",
        "selection_reason": "Predeclared using001: capped24 had fewer occupancy errors; both old policies remain in probe reports.",
        "maximum_epochs": args.epochs, "teaching_signed_score_hz": args.margin_hz,
        "training": "Each epoch presents all60 nonempty capped24 placement contexts and16 Undo contexts once, in a saved seeded order. Frozen500ms observation; teach iff label-signed fixed-decoder score is below the recorded margin. Existing bidirectional targetDAN200 learning/passive250/reset/oppositeDAN200 frozen/cue500 learning/passive250. Otherwise the trial stays fully frozen. Teaching labels never enter inputs or runtime decisions. Passive decay remains active during every nonfrozen window.",
        "evaluation": "Evaluate121 inputs from reset with W/u/w frozen at inheritance and after each epoch; save every memory. All-source response/weight hashes checked at inheritance. Exact full-weight before/after checks for every frozen batch. No independent-sample claim for response reuse.",
        ...
        "gates": {"placement_nonempty_balanced_accuracy": .90, "placement_occupancy_class_recall": .85,
                  "placement_gain_over_original_history_controls": .25, "familiar_retention": 1.0,
    ...
    def score(responses, mapping, prior_run):
        ...
        for policy in POLICIES:
            ...
            gate = (scored["nonempty"]["balanced_accuracy"] >= .90 and
                    all(v >= .85 for n, group in scored["by_distinct_peers"].items() if n != "0" for v in group["class_recall"].values()) and
                    all(scored["nonempty"]["balanced_accuracy"] >= prior_run["arms"][arm]["policies"][policy]["probe"]["nonempty"]["balanced_accuracy"] + .25
                        for arm in ("frozen", "no_feedback", "inconsistent")))
            policies[policy] = {"metrics": scored, "gate": gate}
        ...
        gate = policies["capped-24"]["gate"] and retained == 16 and undo["balanced_accuracy"] >= .90 and min(undo["class_recall"].values()) >= .85
    ...
    with gzip.open(args.out / "episodes.jsonl.gz", "xt", compresslevel=6) as episodes:
        for source_run in source_summary["runs"]:
            ...
            inherited = evaluate({"seed": seed, "mapping": mapping, "phase": "inherited"},
                                 {k: source_recall[seed, mapping, k] for k in inputs})
            history = []
            selected_epoch = None
            for epoch, order in enumerate(schedules[str(seed)], 1):
                ...
                if scored["joint_judgment_gate"] or epoch == args.epochs:
                    ...
                    for case in cases:
                        if case["split"] != "development":
                            continue
                        result = play_with_undo(case, responses, templates, groups, undo_groups, "capped-24", mapping)
                        episodes.write(json.dumps({"seed": seed, "mapping": mapping, "epoch": epoch, "policy": "capped-24", **result}) + "\n")
                ...
                print(json.dumps({"seed": seed, "mapping": mapping, "epoch": epoch, "teaching_events": teaching_events,
                                  "placement": scored["policies"]["capped-24"]["metrics"]["nonempty"]["balanced_accuracy"],
                                  "n4": scored["policies"]["capped-24"]["metrics"]["by_distinct_peers"]["4"]["class_recall"],
    ...
    summary = {"pilot_gate": all(r["selected_epoch"] is not None for r in results), "stage5_complete": False,
               "controls_evaluated": False, "heldout_evaluated": False, "runs": results,
